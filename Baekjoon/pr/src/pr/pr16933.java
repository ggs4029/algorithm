...
// 단 벽은 낮에만 부술수 있음.
// 가장 처음에 이동할 때는 낮이고 한 번 이동할 때 마다 밤낮이 바뀜.
...
// 밤낮에 주의 - 벽이 없으면 상관없지만 벽을 부수는 경우는 낮에만 가능함.
// bfs메소드에 밤낮 구분하는 변수 추가해 벽을 부수는 경우에 체크하도록 수정하기.
// 벽인데 밤이면 하루 그냥 머무르고 낮에 부수고 이동해야함.ㄴ
...
public class pr16933 {
	...
	public static int bfs() {
		...
		while(!q.isEmpty()) {
			Node tmp = q.poll();
			boolean time = tmp.move % 2 != 0;													// 밤낮 구분하기위한 변수
				...
				if(nx>=0 && nx<n && ny>=0 && ny<m) {
					if(map[nx][ny] == 0) {														// 벽이 아니고
						...
						if(tmp.wall < k) {														// 아직 벽 더 부술 수 있고
							if(!visited[nx][ny][tmp.wall+1]) {									// 방문 안했고
								if(time) {														// 낮이면
									visited[nx][ny][tmp.wall+1] = true;							// 부수고 방문
									q.offer(new Node(nx, ny, tmp.move+1, tmp.wall+1));
								}
								else															// 밤이면
									q.offer(new Node(tmp.x, tmp.y, tmp.move + 1, tmp.wall));	// 하루 머무르기
							}
						}
					}
				}
